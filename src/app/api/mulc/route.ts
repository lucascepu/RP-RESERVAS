...
const FILE_PATH = 'src/data/mulc.json';
...
async function ghPush(data: {f:string;v:number;tc:number}[], sha: string, message: string) {
...
export async function POST(req: NextRequest) {
  ...
  if (idx >= 0) {
    ...
    data.push({ f: fecha, v: valor, tc: 0 });
    data.sort((a: {f:string}, b: {f:string}) => a.f.localeCompare(b.f));
  }

  await ghPush(data, sha, `data: MULC ${fecha} = ${valor} MM`);
  revalidatePath('/');
